...
public class AudibleAPIDataGetter : AudibleDataGetter {
    private static readonly string BookResponseGroups = string.Join(",", new List<string> {
                                                                                              "category_media",
                                                                                              "category_metadata",
                                                                                              "category_ladders",
                                                                                              "product_attrs",
    ...
    public async Task<ParseAudioBook> ParseBook(string asin) {
        try {
            ...
            if (json.product.distribution_rights_region != null) {
                ...
                foreach (dynamic? locale in json.product.distribution_rights_region)
                    if (locale == "US") {
                        isAvailable = true;
                        break;
                    }
            ...
            if (runTimeMinutes != null) book.RuntimeSeconds = (int)runTimeMinutes * 60;
            ...
            if (json.product.authors != null)
                foreach (dynamic jsonAuthor in json.product.authors) {
                    ParseAudioBookPerson author = new();
            ...
            if (json.product.narrators != null)
                foreach (dynamic? jsonNarrator in json.product.narrators) {
                    dynamic? narrator = jsonNarrator.name.Value;
            ...
            Dictionary<string, ParseAudioBookCategory> CategoriesToAdd = new();
            if (json.product.category_ladders != null)
                foreach (dynamic ladder in json.product.category_ladders) {
                    ...
                    foreach (dynamic category in ladder.ladder) {
                        ...
                        ParseAudioBookCategory bookCategory = new();
                        bookCategory.Id = id;
                        ...
                        if (!CategoriesToAdd.ContainsKey(id)) CategoriesToAdd.Add(id, bookCategory);

                        latestCategory = name;
                        latestCateGoryLink = currentLink;
                    }

                    if (!book.Tags.Contains(latestCategory)) book.Tags.Add(latestCategory);
                }
            else
