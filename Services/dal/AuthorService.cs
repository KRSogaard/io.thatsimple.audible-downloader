...
namespace AudibleDownloader.Services.dal
{
    public class AuthorService
    {
        ...
        {
            return MSU.Query("SELECT * FROM `authors` WHERE `asin` = @asin",
        ...
        {
            return MSU.Query("SELECT * FROM `authors` WHERE `id` = @id",
        ...
        {
            bool exists = await MSU.Query(
                "SELECT * FROM `authors_books` WHERE `book_id` = @bookId AND `author_id` = @authorId",
                new Dictionary<string, object> { { "@bookId", bookId }, { "@authorId", author.Id } },
                async (MySqlDataReader reader) => { return await reader.ReadAsync() == true; });
            if (!exists)
            {
                log.Debug($"Adding book {bookId} to author {author.Id}");
                await MSU.Execute(
                    "INSERT INTO `authors_books` (`book_id`, `author_id`, `created`) VALUES (@bookId, @authorId, @created)",
                ...
                await MSU.Execute(
                    "UPDATE `books` SET `last_updated` = @lastUpdate, `authors_cache` = concat(ifnull(`authors_cache`,\"\"), @cache) WHERE `id` = @bookId",
                    new Dictionary<string, object>
                    {
        ...
        public async Task<AudibleAuthor> SaveOrGetAuthor(string asin, string name, string link)
        {
            ...
            return await MSU.QueryWithCommand(
                "INSERT INTO `authors` (`name`, `asin`, `link`, `created`) VALUES (@name, @asin, @link, @created)",
                new Dictionary<string, object>
                {
                    { "@name", name }, { "@asin", asin }, { "@link", link },
                    { "@created", DateTimeOffset.Now.ToUnixTimeSeconds() }
                },
