...
public class AudibleScraperDataGetter : AudibleDataGetter
{
    ...
    public async Task<ParseAudioBook> ParseBook(string asin)
    {
        ...
        var downloadResponse = await downloadService.DownloadHtml(url, message =>
        {
            if (message.Headers.Location != null)
            {
                url = "https://www.audible.com" + message.Headers.Location.ToString();
                if (url.StartsWith("https://www.audible.com/pd/") && url.EndsWith(asin))
                {
                    return true;
                }
    ...
    public async Task<ParseSeries> ParseSeries(string asin)
    {
        ...
        var downloadResponse = await downloadService.DownloadHtml(url, message =>
        {
            ...
            {
                url = "https://www.audible.com" + message.Headers.Location.ToString();
                if (url.StartsWith("https://www.audible.com/series/") && url.EndsWith(asin))
                {
                    return true;
                }
                log.Debug("Redirection to unexpected url " + url + " from " + url);
